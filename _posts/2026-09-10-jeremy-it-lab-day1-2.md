...
4. UTP, 핀아웃 규격 및 케이블 종류
  ...
  5. 광섬유 케이블(Fiber-Optic Cables)
    - 전기 신호 대신 빛을 이용하여 데이터를 전송하므로 EMI 영향을 받지 않고 휠씬 먼 거리를 전송할 수 있다. 
        - MMF (Multimode Fiber, 멀티모드) : 코어 직경이 크고 저렴한 LED, 레이저 광원을 상요하여 여러 빛 경로가 발생하여 신호 왜곡이 발생하므로 수백미터의 단거리 연결에서 사용됨
